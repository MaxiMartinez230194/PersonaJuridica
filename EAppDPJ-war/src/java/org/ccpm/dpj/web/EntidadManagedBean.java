...
@ManagedBean
@RequestScoped
public class EntidadManagedBean extends UtilManagedBean implements Serializable{
    @EJB
        ...
        try {
            ...
            if (!(lstActionItems.isEmpty())) {
                for (ActionItem accionAux : lstActionItems) {
                    if (accionAux.getNombre().equalsIgnoreCase("nuevaEntidad")) {
                        this.setAlta(true);
                    }
                    if (accionAux.getNombre().equalsIgnoreCase("editarEntidad")) {
                        this.setModificacion(true);
                    }
                    if (accionAux.getNombre().equalsIgnoreCase("borrarEntidad")) {
                        this.setBaja(true);
                    }
                    if (accionAux.getNombre().equalsIgnoreCase("detalleEntidad")) {
    ...
    public List <Entidad> getListElements() {
     ...
     public List<SelectItem> getSelectItemsEntidades() {
        List <SelectItem> selectItems = new ArrayList <SelectItem>();
        List <Entidad> lstEntidad = this.getListElements();
        if (!(lstEntidad.isEmpty())) {
            for (Entidad EntidadAux : lstEntidad) {
                selectItems.add(new SelectItem(EntidadAux.getId(), EntidadAux.getNombre()));
            }
        }
        return selectItems;
    }
    ...
    public void limpiar() {
        ...
        this.setIdTipoEntidad(0L);
    }

    @Override
    public String crear() {
        try {
            this.entidadFacade.create(this.getNombre(), this.getCodigo(), this.getIdMunicipio(), this.getDireccion(), this.getTelefono(), this.getCorreo(), this.getPaginaWeb(), this.getIdEstado(), this.getFechaAlta(), 1L, this.getFechaVencimiento(fechaAlta),this.getDni(), this.getNombre());
            this.setResultado("successErrorEntidad");
    ...
    public String crearOtro() {
        try {
            this.entidadFacade.create(this.getNombre(), this.getCodigo(), this.getIdMunicipio(), this.getDireccion(), this.getTelefono(), this.getCorreo(), this.getPaginaWeb(), this.getIdEstado(), this.getFechaAlta(), 1L, this.getFechaVencimiento(fechaAlta),this.getDni(), this.getNombre());
            this.setResultado("nuevaEntidad");
            this.limpiar();
        } catch (Exception ex) {
            this.setTitle("Resultado del Chequeo...");
            this.setImages("glyphicon glyphicon-remove-circle");
    ...
    public String guardarEdicion() {
        try {
            this.entidadFacade.edit(this.getId(), this.getNombre(), this.getCodigo(), this.getIdMunicipio(), this.getDireccion(), this.getTelefono(), this.getCorreo(), this.getPaginaWeb(), this.getIdEstado(), this.getFechaAlta(), 1L, this.getFechaVencimiento(fechaAlta),this.getDni(), this.getNombre());
            this.setTitle("Proceso Completo...");
            this.setImages("glyphicon glyphicon-ok-circle");
            this.setResultado("successErrorEntidad");
